...
/**
 * This class takes an user search terms and parses Javauto.java for relevant
...
public class Lookup {
	...
	public static void main(String[] args) {
		...
		for (String arg : args) {
			if (arg.equals("-n") || arg.equals("--name")) {
				...
				if (!found)
					System.out.println("No results found for: " + GREEN + arg
							+ NORMAL);
				docDisplay = false;
			} else if (termSearch) {
				...
				for (String[] f : funcData) {
					if (f[0].toLowerCase().startsWith(arg.toLowerCase()))
						...
						sResults.add(f);
					else if (f[2].toLowerCase().contains(arg.toLowerCase())
							|| f[3].toLowerCase().contains(arg.toLowerCase()))
						sResults.add(f);
				}
				...
				if (pResults.size() == 0 && sResults.size() == 0
						&& varDisplayed == false)
					System.out.println("No results found for: " + GREEN + arg
							+ NORMAL);
				termSearch = false;
			...
			/* if the user is performing a search just do that and then exit */
				...
				/*
				 * search function names for ones that equal or begin with our
				 * term
				 */
				for (String[] f : funcNames) {
					/*
					 * get the function name with only lower case
					 * letters/numbers
					 */
					String func = f[0].replaceAll("[^a-zA-Z0-9]", "")
							.toLowerCase();

					/*
					 * if it starts with/equals our search term add it to
					 * results
					 */
					if (func.startsWith(term.replaceAll("[^a-zA-Z0-9]", "")
							.toLowerCase()))
						pResults.add(f[0]);

					/*
					 * if it contains our search term add it to secondary
					 * results
					 */
					else if (func.trim().contains(
							term.replaceAll("[^a-zA-Z0-9]", "").toLowerCase()))
						sResults.add(f[0]);
				}

				/*
				 * search class variables for ones that equal or begin with our
				 * term
				 */
				for (String v : varNames) {
					/*
					 * get the variable name with only lower case
					 * letters/numbers
					 */
					v = v.split("=")[0].trim();
	...
	private static void displayFunction(String[] data) {
		...
		System.out.println(BLUE
				+ data[2].replaceAll("\n", "\n\t").split("@param")[0]
						.split("@return")[0].trim() + NORMAL);
		System.out.println("\t" + GREEN + data[3].replaceAll("\n", "\n\t")
				+ NORMAL);
		System.out.println("\tReturn type:  " + YELLOW + data[1].trim()
				+ NORMAL);
		if (!data[1].equals("void"))
			System.out.println("\tReturn value: " + YELLOW
					+ data[2].split("@return")[1].trim() + NORMAL);
		System.out.println("");
	}
	 ...
	 * Gets the var data.
	 *
	 * @param funcFileContents
	 *            the func file contents
	 * @return the var data
	 */
	private static ArrayList<String[]> getVarData(String funcFileContents) {
		/*
		 * ArrayList to hold variable data, each item having format [var name,
		 * description]
		 */
		...
		for (int i = lines.length - 1; i >= 0; i--) {
			/*
			 * if it looks like a function declaration and the previous line has
			 * an ending javadoc comment
			 */
			if (lines[i]
					.matches("^\\s*public\\s+[\\S+\\s+]*\\s+(\\w+)\\s*=.*;$")
					&& lines[i - 1].matches(".*\\*/\\s*$")) {
	...
	/**
	 * Returns data about each function defined in an ArrayList<String[]> Where
	 * each index is like [name, return type, javadoc, signatures].
	 *
	 * @param funcFileContents
	 *            the func file contents
	...
	private static ArrayList<String[]> getFuncData(String funcFileContents) {
		/*
		 * ArrayList to hold function data in the form [name, return type,
		 * javadoc, signatures]
		 */
		...
		for (int i = lines.length - 1; i >= 0; i--) {
			...
			if (lines[i]
				...
				/*
				 * figure out if our function is already in the data and we're
				 * adding a new signature/doc comment
				 */
				...
				if (!contains)
					funcData.add(new String[] { name, returnType, jdComment,
							signature });
			}
		}
		return funcData;
